...
public class SledgehammerModelGen extends ItemModelProvider {
    ...
    private void genSledgehammerModels(String hammerMatName) {
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/" + hammerMatName + "_sledgehammer");

        ItemModelBuilder card = factory.apply(location);
        card.parent(this.factory.apply(ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/base_sledgehammer")));
        card.texture("base", ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/" + hammerMatName + "_sledgehammer"));
        ...
        for (int m = 0; m < TRIM_MATERIALS.length; m++) {
            ...
            for (int t = 0; t < TRIM_TYPES.length; t++) {
                ...
                cardOverride.predicate(ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "trim_material"), TRIM_MATERIALS_INDEXES[m]);
                cardOverride.predicate(ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "trim_type"), t + 1);
    ...
    private ModelFile genSledgehammerTrim(String hammerMatName, String trimMatName, String trimTypeName) {
        ...
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/sledgehammers/" + hammerMatName + "/" + hammerMatName + "_sledgehammer_" + trimMatName + "_" + trimTypeName);

        ItemModelBuilder card = factory.apply(location);
        card.parent(factory.apply(ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/base_sledgehammer")));
        card.texture("base", ResourceLocation.fromNamespaceAndPath(SledgehammersRef.MOD_ID, "item/" + hammerMatName + "_sledgehammer"));
