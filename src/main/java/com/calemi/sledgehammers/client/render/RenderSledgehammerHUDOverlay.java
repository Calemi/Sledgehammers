...
public class RenderSledgehammerHUDOverlay {
    ...
    public void onRenderGameOverlayEvent(RenderGuiLayerEvent.Post event) {
        ...
        if (activeItemStack.getItem() instanceof SledgehammerItem sledgehammer) {
            ...
            RenderSystem.setShaderTexture(0, SledgeRef.HAMMER_OVERLAY);

            //If the Sledgehammer is charging, render the loading charge overlay.
            if (player.getTicksUsingItem() < chargeTime) {

                //BACKGROUND HAMMER
                event.getGuiGraphics().blit(SledgeRef.HAMMER_OVERLAY, midX - 7, midY - 11, 0, 0, hammerIconWidth, hammerIconHeight);

                //CHARGING HAMMER
                event.getGuiGraphics().blit(SledgeRef.HAMMER_OVERLAY, midX - 7, midY - 11, hammerIconWidth, 0, scaledChargeTime, hammerIconHeight);
            }
            ...
            //If the Sledgehammer is ready, render the flashing ready overlay.
                ...
                if (level.getGameTime() % 5 > 1) {
                    //FLASHING HAMMER
                    event.getGuiGraphics().blit(SledgeRef.HAMMER_OVERLAY, midX - 7, midY - 11, hammerIconWidth * 2, 0, hammerIconWidth, hammerIconHeight);
                }
            }
        }
